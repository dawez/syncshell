<script>
    ...
    $effect(() => {
        ...
        api.get('db/remoteneed', {folder: folderID, device: deviceID, page, perpage}, controller.signal)
            .then(data => { files = data.files || []; error = ''; }).catch(failure => { if (!controller.signal.aborted) error = failure.message; });
...
<details class="panel panel-default" open={single}>
    ...
    <div class="panel-body less-padding">
        ...
        <table class="table table-striped"><thead><tr>{#each ['Path', 'Size', 'Mod. Time', 'Mod. Device'] as label}<th title={label === 'Mod. Time' ? locale.t('Time the item was last modified') : label === 'Mod. Device' ? locale.t('Device that last modified the item') : undefined}>{locale.t(label)}</th>{/each}</tr></thead>
            <tbody>{#each files as file}<tr><td class="word-break-all">{file.name}</td><td>{['DIRECTORY', 'FILE_INFO_TYPE_DIRECTORY'].includes(file.type) ? '' : unitPrefixed(file.size, true) + 'B'}</td><td>{timestamp(file.modified)}</td><td>{friendly(file.modifiedBy)}</td></tr>{/each}</tbody>
        </table>
