...
{#snippet full()}
    <div><span><span class="far fa-fw fa-copy"></span> {locale.t('Files')}:</span><span>{(files || 0).toLocaleString()}</span></div>
    <div><span><span class="far fa-fw fa-folder"></span> {locale.t('Folders')}:</span><span>{(folders || 0).toLocaleString()}</span></div>
    <div><span><span class="far fa-fw fa-hdd"></span> {locale.t('Total')}:</span><span>~{unitPrefixed(bytes, true)}B</span></div>
{/snippet}
