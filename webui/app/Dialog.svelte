<script>
    ...
    let {title, large = false, status = 'default', icon = '', children, footer, onClose} = $props();
    const locale = getContext('locale');
    let dialog;
    onMount(() => { dialog.showModal(); return () => dialog.close(); });
</script>

<dialog bind:this={dialog} class="port-dialog" class:large aria-label={locale.t(title)} onclose={onClose}>
